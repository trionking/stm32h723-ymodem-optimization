...
void SDMMC1_IRQHandler(void)
{
  ...
  static volatile uint32_t sdmmc_irq_count = 0;
  sdmmc_irq_count++;

  // SDMMC 인터럽트 발생 로그 (매번 출력하면 너무 많으니 처음 몇 번만)
  if (sdmmc_irq_count <= 5) {
    extern int printf(const char *format, ...);
    printf("[IRQ] SDMMC1_IRQHandler called! count=%lu\r\n", sdmmc_irq_count);
  }
...
/**
  * @brief UART TX Complete Callback (DMA 전송 완료)
  */
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART2) {
        // user_def.c의 UART2 TX Complete Callback 호출
